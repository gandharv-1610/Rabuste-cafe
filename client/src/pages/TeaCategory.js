  ...
  const fetchTeas = async () => {
    try {
      const response = await api.get('/coffee', {
        params: { category: 'Tea', _t: Date.now() },
      });
  ...
  if (loading) {
    return (
      <div className="pt-20 min-h-screen flex items-center justify-center">
        <div className="text-coffee-amber text-xl">Loading tea...</div>
      </div>
    );
  }
      ...
      <section className="py-12 px-4 bg-gradient-to-b from-coffee-darker to-coffee-dark">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <p className="text-coffee-light text-lg">
            Handpicked teas to soothe and refresh
          </p>
        </div>
      </section>
      ...
      <section className="py-20 px-4 max-w-6xl mx-auto">
        {teas.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-coffee-light text-lg">No tea items available yet.</p>
          </div>
        ) : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {teas.map((tea, idx) => {
              const isFlipped = flippedCards.has(tea._id);
              return (
                <motion.div
                  key={tea._id}
                  ...
                  onClick={(e) => {
                    if (window.innerWidth < 768 || 'ontouchstart' in window) {
                      ...
                      setFlippedCards(prev => {
                        ...
                        if (newSet.has(tea._id)) {
                          newSet.delete(tea._id);
                        } else {
                          newSet.add(tea._id);
                        }
                        return newSet;
                ...
                >
                  <div className="flip-card-inner">
                    ...
                    <div className="flip-card-front">
                      {(tea.image || tea.cloudinary_url) ? (
                        <div className="w-full h-48 bg-coffee-brown/40 overflow-hidden rounded-lg mb-4">
                          <img
                            src={`${tea.cloudinary_url || tea.image}?v=${tea.updatedAt || Date.now()}`}
                            alt={tea.name}
                      ...
                      <div className="w-full">
                        ...
                        <div className="mt-auto">
                          <p className="text-coffee-amber font-bold text-xl">₹{tea.price?.toFixed(2) || '0.00'}</p>
                        </div>
                      </div>
                        ...
                        {tea.flavorNotes && tea.flavorNotes.length > 0 && (
                          <div className="mt-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {tea.flavorNotes.map((note, noteIdx) => (
                                <span
                                  key={noteIdx}
